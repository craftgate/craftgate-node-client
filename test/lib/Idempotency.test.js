...
test('post() sends the idempotency key from the header options as a header', async t => {
  const client = newClient();
  const seen = capture(client, 'Post', '/foo');

  await client.post('/foo', { foo: 42, headerOptions: { idempotencyKey: 'idempotency-key-1' } });
...
test('post() omits the header when the header options are empty', async t => {
  const client = newClient();
  const seen = capture(client, 'Post', '/foo');

  await client.post('/foo', { foo: 42, headerOptions: {} });

  t.false(Object.prototype.hasOwnProperty.call(seen.headers, IDEMPOTENCY_KEY_HEADER_NAME));
});

test('post() strips the header options from the serialized body', async t => {
  const client = newClient();
  const seen = capture(client, 'Post', '/foo');
  ...
  t.false(seen.data.includes('headerOptions'));
  t.false(seen.data.includes('idempotencyKey'));
  t.false(seen.data.includes('idempotency-key-1'));
});

test('post() strips only the top-level header options, leaving nested caller data untouched', async t => {
  const client = newClient();
  const seen = capture(client, 'Post', '/foo');
  ...
  await client.post('/foo', {
    ...
    additionalParams: { headerOptions: { idempotencyKey: 'provider-side-key' } }
  });

  t.is(seen.headers[IDEMPOTENCY_KEY_HEADER_NAME], 'idempotency-key-1');
  t.is(seen.data, JSON.stringify({ foo: 42, additionalParams: { headerOptions: { idempotencyKey: 'provider-side-key' } } }));
  t.false(seen.data.includes('idempotency-key-1'));
});

test('post() signs the same body with and without header options', async t => {
  utils.generateRandomString = () => 'foo';
  try {
    const withKey = newClient();
    const seenWithKey = capture(withKey, 'Post', '/foo');
    await withKey.post('/foo', { foo: 42, headerOptions: { idempotencyKey: 'idempotency-key-1' } });
...
test('get() sends the key as a header and keeps it out of the query string', async t => {
  ...
  await client.get('/foo', { foo: 42, headerOptions: { idempotencyKey: 'idempotency-key-1' } });

  t.is(seen.headers[IDEMPOTENCY_KEY_HEADER_NAME], 'idempotency-key-1');
  t.is(utils.serializeParams({ foo: 42, headerOptions: { idempotencyKey: 'idempotency-key-1' } }), 'foo=42');
});

test('serializeParams omits the reserved header options key', t => {
  t.is(utils.serializeParams({ headerOptions: { idempotencyKey: 'idempotency-key-1' } }), '');
  t.is(utils.serializeParams({ a: 1, headerOptions: { idempotencyKey: 'idempotency-key-1' }, b: 2 }), 'a=1&b=2');
});

test('get() signs the same URL with and without header options', async t => {
  utils.generateRandomString = () => 'foo';
  try {
    const withKey = newClient();
    const seenWithKey = capture(withKey, 'Get', '/foo');
    await withKey.get('/foo', { foo: 42, headerOptions: { idempotencyKey: 'idempotency-key-1' } });
...
test('delete() with requestScopedConfig sends the header and no body', async t => {
  ...
  await client.delete('/foo', undefined, requestScopedConfig({ headerOptions: { idempotencyKey: 'idempotency-key-1' } }));
...
test('delete() signature is unchanged by the key', async t => {
  ...
  try {
    ...
    await withKey.delete('/foo', undefined, requestScopedConfig({ headerOptions: { idempotencyKey: 'idempotency-key-1' } }));
...
test('requestScopedConfig returns an empty config when no option is set', t => {
  ...
  t.deepEqual(requestScopedConfig({ headerOptions: undefined }), {});
  t.deepEqual(requestScopedConfig({ headerOptions: {} }), {});
  t.deepEqual(requestScopedConfig({ headerOptions: { idempotencyKey: undefined } }), {});
});
...
test('the caller request object is not mutated, so it can be reused to retry', async t => {
  ...
  const request = { foo: 42, headerOptions: { idempotencyKey: 'idempotency-key-1' } };
  await client.post('/foo', request);

  t.is(request.headerOptions.idempotencyKey, 'idempotency-key-1');
  t.deepEqual(request, { foo: 42, headerOptions: { idempotencyKey: 'idempotency-key-1' } });
});
